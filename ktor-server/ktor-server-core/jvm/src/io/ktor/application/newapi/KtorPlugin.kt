...
public fun <A : Pipeline<*, ApplicationCall>> A.plugin(
    plugin: KtorInstallablePlugin<Configuration>
...
internal fun <A : Pipeline<*, ApplicationCall>> A.findInterceptionsHolder(
    plugin: KtorInstallablePlugin<*>
): InterceptionsHolder {
    return attributes[featureRegistryKey].getOrNull(plugin.key)
        ?: throw MissingApplicationFeatureException(plugin.key)
}

public abstract class KtorInstallablePlugin<Configuration : Any>(public val name: String) :
...
public abstract class KtorPlugin<Configuration : Any> private constructor(
    installablePlugin: KtorInstallablePlugin<Configuration>
) : PluginContext,
    InterceptionsHolder {
    ...
    override val key: AttributeKey<KtorPlugin<Configuration>> = installablePlugin.key
    ...
    public abstract class RelativePluginContext(
        private val currentPlugin: KtorPlugin<*>,
        ...
        override val onRequest: OnRequest = object : OnRequest {
            override operator fun invoke(block: suspend RequestContext.(ApplicationCall) -> Unit) {
                insertToPhaseRelatively(
                    currentPlugin.callInterceptions, otherPlugin.callInterceptions,
                    ::RequestContext,
                    block
                )
            }
    ...
    public class AfterPluginContext(currentPlugin: KtorPlugin<*>, otherPlugin: InterceptionsHolder) :
    ...
    public class BeforePluginContext(currentPlugin: KtorPlugin<*>, otherPlugin: InterceptionsHolder) :
        RelativePluginContext(currentPlugin, otherPlugin) {
    ...
    public fun afterPlugin(
        installablePlugin: KtorInstallablePlugin<out Any>,
    ...
    public companion object {
        ...
        public fun <Configuration : Any> createPlugin(
        ...
        ): KtorInstallablePlugin<Configuration> = object : KtorInstallablePlugin<Configuration>(name) {
            ...
            override fun install(
                ...
                val self = this
                val pluginInstance = object : KtorPlugin<Configuration>(self) {
        ...
        public fun createPlugin(
            name: String,
            body: KtorPlugin<Unit>.() -> Unit
        ): KtorInstallablePlugin<Unit> = createPlugin<Unit>(name, {}, body)
    }
